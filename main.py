...
def fitness(X):
    return np.array([reward(X[i]) for i in range(X.shape[0])])
...
optimizer = ps.single.LocalBestPSO(n_particles=100, dimensions=(2 * dimension + 1) * number_of_rules + 1,
                                   options=options)
cost, pos = optimizer.optimize(fitness, print_step=100, iters=10000, verbose=3)
pickle.dump({"cost": cost, "best": pos}, "result.pkl")
