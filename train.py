...
initial_states = np.random.uniform(-0.5, 0.5, (number_of_episodes, dimension))
initial_states[:, 1] = initial_states[:, 3] = np.zeros(number_of_episodes)


def run_episode(observation, fuzzy_system: FuzzySystem):
    env.reset()
    total_reward = 0
...
def particle_reward(params):
    ...
    for i in range(number_of_episodes):
        total_reward += run_episode(initial_states[i], fuzzy_system)
